...
void MonoOdometerOmnidirectional::imageCallback(const sensor_msgs::msg::Image::ConstSharedPtr& image_msg)
{
  ...
  if (first_run) {
    ...
    if(success) {
      ...
      RCLCPP_DEBUG_STREAM(this->get_logger(), 
        "libviso2 returned the following motion:\n" << camera_motion);
      ...
      RCLCPP_DEBUG(this->get_logger(), 
        "Call to VisualOdometryMono::process() failed. Assuming motion too small.");
      replace_ = true;
      tf2::Transform delta_transform;
      delta_transform.setIdentity();
      integrateAndPublish(delta_transform, image_msg->header.stamp);
